 ...
 */
export const getCurrentUser = async (_, session) => {
  if (!session) return

  const { valid, data } = verifyToken(session.token)
  if (!valid) return
  if (!data) return
...
const verifyToken = (token: string) => {
  try {
    ...
    const data = jwt.verify(token, process.env.TOKEN_SIGN_KEY!) as jwt.JwtPayload
